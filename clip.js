...
var geoUtil = {
     ...
     * 注:线与多边形必须有两个交点
     */
    polygonClipByLine: function (polygon, clipLine,isMercator) {
        if(isMercator){
        ...
        if (polygon.geometry.type === 'Polygon') {
            ...
            if (polyLine.geometry.type === 'LineString') { // 切割普通多边形
                return this._singlePolygonClip(polyLine, clipLine,isMercator);
            } else if (polyLine.geometry.type === 'MultiLineString') { //切割环
                return this._multiPolygonClip(polyLine, clipLine,isMercator);
            }
    ...
    _singlePolygonClip: function (polyLine, clipLine,isMercator) {
        // 获得裁切点
        var intersects = turf.lineIntersect(polyLine, clipLine);
        if (intersects.features.length !== 2) {
            throw {
                state: '裁剪失败',
                message: '切割线与多边形交点应该为2个,当前交点个数为' + intersects.features.length
            };
        }
        ...
        // 返回面要素集
        if(isMercator){
            return turf.toMercator(turf.featureCollection([
                resultPolygon1,
                resultPolygon2
            ]));
        }else{
    ...
    _multiPolygonClip: function (polyLine, clipLine,isMercator) {
        ...
        if(isMercator){
            return turf.toMercator(resultCollection);
        }else{
